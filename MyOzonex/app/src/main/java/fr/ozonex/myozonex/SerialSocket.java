...
class SerialSocket extends BluetoothGattCallback {
    ...
    private static final UUID BLUETOOTH_LE_MYOZONEX_SERVICE     = UUID.fromString("0000A002-0000-1000-8000-00805F9B34FB");
    private static final UUID BLUETOOTH_LE_MYOZONEX_CHAR_TX     = UUID.fromString("0000C303-0000-1000-8000-00805F9B34FB"); // W
    private static final UUID BLUETOOTH_LE_MYOZONEX_CHAR_RX     = UUID.fromString("0000c306-0000-1000-8000-00805F9B34FB"); // N
    ...
    private void onPairingBroadcastReceive(Context context, Intent intent) {
        ...
        switch (intent.getAction()) {
            case BluetoothDevice.ACTION_PAIRING_REQUEST:
                ...
                onSerialConnectError(new IOException(context.getString(R.string.bluetooth_appairage_requis)));
    ...
    private void connectCharacteristics1(BluetoothGatt gatt) {
        ...
        for (BluetoothGattService gattService : gatt.getServices()) {
            ...
            if (gattService.getUuid().equals(BLUETOOTH_LE_MYOZONEX_SERVICE))
                delegate = new MyOzonexDelegate();
    ...
    private class MyOzonexDelegate extends DeviceDelegate {
        @Override
        boolean connectCharacteristics(BluetoothGattService gattService) {
            BluetoothGattCharacteristic rw2 = gattService.getCharacteristic(BLUETOOTH_LE_MYOZONEX_CHAR_TX);
            BluetoothGattCharacteristic rw3 = gattService.getCharacteristic(BLUETOOTH_LE_MYOZONEX_CHAR_RX);
            if (rw2 != null && rw3 != null) {
                int rw2prop = rw2.getProperties();
                int rw3prop = rw3.getProperties();
                boolean rw2write = (rw2prop & BluetoothGattCharacteristic.PROPERTY_WRITE_NO_RESPONSE) != 0;
                boolean rw3write = (rw3prop & BluetoothGattCharacteristic.PROPERTY_WRITE_NO_RESPONSE) != 0;
                if (rw2write && rw3write) {
                    onSerialConnectError(new IOException("multiple write characteristics (" + rw2prop + "/" + rw3prop + ")"));
                } else if (rw2write) {
                    writeCharacteristic = rw2;
